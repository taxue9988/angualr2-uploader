export class Exif {
    ...
    public addEvent(element, event, handler) {
    ...
    public imageHasData(img) {
        return !!(img.exifdata);
    }


    public base64ToArrayBuffer(base64, contentType?: any) {
    ...
    public objectURLToBlob(url, callback) {
    ...
    public getImageData(img, callback) {
        let self = this;
        function handleBinaryFile(binFile) {
            var data = self.findEXIFinJPEG(binFile);
            var iptcdata = self.findIPTCinJPEG(binFile);
            img.exifdata = data || {};
        ...
        if (img instanceof Image || img instanceof HTMLImageElement) {
            ...
            var fileReader = new FileReader();
            fileReader.onload = function (e:any) {
                if (self.debug) console.log("Got file of length " + e.target.result.byteLength);
                handleBinaryFile(e.target.result);
            };
    ...
    public findEXIFinJPEG(file) {
        ...
        while (offset < length) {
            if (dataView.getUint8(offset) != 0xFF) {
                if (this.debug) console.log("Not a valid marker at offset " + offset + ", found: " + dataView.getUint8(offset));
            ...
            if (marker == 225) {
                ...
                return self.readEXIFData(dataView, offset + 4, dataView.getUint16(offset + 2) - 2);
    ...
    public readIPTCData(file, startOffset, sectionLength) {
        var dataView = new DataView(file);
        var data = {};
    ...
    public readTags(file, tiffStart, dirStart, strings, bigEnd) {
    ...
    public readTagValue(file, entryOffset, tiffStart, dirStart, bigEnd) {
    ...
    public getStringFromDB(buffer, start, length) {
        var outstr = "";
    ...
    public readEXIFData(file, start,args) {
        ...
        if (firstIFDOffset < 0x00000008) {
            if (this.debug) console.log("Not valid TIFF data! (First offset less than 8)", file.getUint32(tiffOffset + 4, !bigEnd));
            return false;
        }
